app.controller('DashboardController',
  ['$window', '$state', '$scope', '$rootScope', '$interval', 'colorService', 'Auth', 'Transactions',
  function($window, $state, $scope, $rootScope, $interval, colorService, Auth, Transactions){
...
app.controller('MenuController',
  ['$window', '$state', '$scope', '$rootScope', '$interval', 'colorService', 'Auth', 'Notifications',
  function($window, $state, $scope, $rootScope, $interval, colorService, Auth, Notifications){
  ...
  var id_channel=$scope.user.user.id.toString();

  $rootScope.loadNotifications=function(){
    if($scope.next && !$scope.load){
      $scope.load=true;
      Notifications.get({page:$scope.next},function (response) {
        angular.forEach(response.results, function(value, key){
          $rootScope.notifications.push(response.results[key]); 
        });
  ...
  var ably = new Ably.Realtime('IdGeHg.HinOhw:M13HoeLS7pcwjQdz'),
    channel = ably.channels.get(id_channel);

  channel.subscribe(id_channel, function(message) {
    console.log(JSON.parse(message.data));
    var message=JSON.parse(message.data);
    $rootScope.notifications.splice(0,0, message);
    $rootScope.$apply();
  });
